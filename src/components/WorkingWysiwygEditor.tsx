...
interface WorkingWysiwygEditorProps {
  value: string;
...
function WorkingWysiwygEditor({ value, onChange, placeholder, onKeyDown }: WorkingWysiwygEditorProps) {
  ...
  const getInitialValue = () => {
    ...
    for (const line of lines) {
      if (line.startsWith('# ')) {
        ...
        blocks.push({ type: ELEMENT_PARAGRAPH, children: [{ text: line }] });
      }
    }
  ...
  // Ensure editor has proper methods
  useEffect(() => {
    if (editor && !(editor as any).addMark) {
      // Add Slate methods if missing
      Object.assign(editor, {
        addMark: Editor.addMark.bind(null, editor as any),
        removeMark: Editor.removeMark.bind(null, editor as any),
      });
    }
  }, [editor]);
...
export default WorkingWysiwygEditor;